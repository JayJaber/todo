...
export default function Tasks ({tasks, onTasksChange, color}) {
  ...
  return (
    <div className={color+' lighten-5'} style={{textAlign: 'center', margin: '1em', padding: '0.5em', paddingTop: '1em', paddingBottom: 0}}>
      ...
      {tasks.length === 0? <>
          <h6 style={{padding: '0.65em'}}>No tasks yet, add some below!</h6>
          <Col s={12} m={12}><div  style={{borderTop: '1px solid rgb(158, 158, 158)', margin: '0 0.6em', paddingTop: '0.07em'}}></div></Col>
        </>
      : <Row style={{marginBottom: '0', marginTop: 0}}>
        ...
        {tasks.map((task, i) => 
          <>
            <Col s={8} m={10} l={10} xl={11} style={{textAlign: 'left', marginTop: (i===0? '0.25em':'1em'), marginBottom: '0.5em', overflowWrap: 'break-word'}}>{task.done? <s>{i+1}.&nbsp;{task.value}</s> : <>{i+1}. {task.value}</> }</Col>
            <Col s={4} m={2} l={2} xl={1} style={{marginTop: (i===0? '0.25em':'1em'), marginBottom: '0.5em', textAlign: 'right'}}><Checkbox label={task.done? 'Done': 'Todo'} checked={task.done} value='done'
